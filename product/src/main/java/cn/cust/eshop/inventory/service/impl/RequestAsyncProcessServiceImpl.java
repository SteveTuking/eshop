...
public class RequestAsyncProcessServiceImpl implements RequestAsyncProcessService {
	...
	public void process(RequestCommand requestCommand) {
		try {
			...
			ArrayBlockingQueue<RequestCommand> queue = getRoutingQueue(requestCommand.getProductId());
			// 将请求放入对应的队列中，完成路由操作
			queue.put(requestCommand);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	 ...
	 * @return 内存队列
	 */
	private ArrayBlockingQueue<RequestCommand> getRoutingQueue(Integer productId) {
		RequestCommandQueue requestQueue = RequestCommandQueue.getInstance();
